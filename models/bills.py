...
Supports activation periods for seasonal/temporary bills
"""
...
class Bill(Base):
    ...
    def is_active_on(self, check_date: date) -> bool:
        """
        Check if bill was active on a specific date.

        Args:
            check_date: The date to check (date object or ISO string)

        Returns:
            True if bill was active on that date, False otherwise
        """
    ...
    def is_currently_active(self) -> bool:
        """Check if bill is currently active (as of today)"""
    ...
    def can_deactivate(self) -> tuple[bool, str]:
        """
        Check if this bill can be deactivated.
        ...
        if not self.is_currently_active:
            return False, "This bill is already inactive."
